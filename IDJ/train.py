...
def load_model(save_dir, num_classes):
...
def train(data_dir, model_dir, args):
    ...
    model = load_model(f"{save_dir}", num_classes).to(device)
    model = torch.nn.DataParallel(model)
    ...
    val_set, train_set = dataset.split_dataset()
    ...
    for epoch in range(args.epochs//5):
        ...
        for idx, train_batch in enumerate(train_loader):
            ...
            if (idx + 1) % args.log_interval == 0:
                ...
                wandb.log({"Train2/loss" : train_loss, "Train2/accuracy" : train_acc})
                loss_value = 0
                matches = 0
        ...
        with torch.no_grad():
            ...
            wandb.log({"Val2/loss":val_loss, "Val2/accuracy":val_acc, "results2":figure})
            print()
